...
const RecentAllies = ({ matchHistory, summonerData }) => {
  ...
  const createData = (riotIdGameName, wins, winrate) => {
    return { riotIdGameName, wins, winrate };
  };
  let rows = [];
  useEffect(() => {
    ...
    rows = allies.map((ally) =>
      createData(
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <SummonerName
            summonerName={ally.riotIdGameName}
            tagLine={ally.riotIdTagline}
          />
        </Stack>,
        <Stack direction="row" alignItems="center" spacing={1}>
          <WinLoseBar wins={ally.wins} losses={ally.gamesPlayed - ally.wins} />
          <span>{Math.round((ally.wins / ally.gamesPlayed) * 100) + "%"}</span>
        </Stack>
      )
    );
  }
