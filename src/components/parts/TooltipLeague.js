...
const TooltipLeague = ({ children, content }) => {
  ...
  if (attentionMatches) {
    for (let i = 0; i < attentionMatches.length; i++) {
      attentions.push({
        ...
        detail: attentionDetailMatches[i]?.match(/>(.*?)</g)[0].slice(2, -1),
      });
    }
  }
  ...
  if (activeMatches) {
    for (let i = 0; i < activeMatches.length; i++) {
      actives.push({
        ...
        detail: activeDetailMatches
          ? activeDetailMatches[i]
              ?.match(/>(.*?)</g)
              .map((match) => match.slice(1, -1))
          : "Something isn't working, fix later",
      });
    }
  }
  console.log(actives);
  ...
  const passiveDetailMatches = content?.description.match(passiveDetailPattern);
  let passives = [];
  if (passiveMatches) {
    ...
    <HtmlTooltip
      ...
      title={
        <React.Fragment>
          ...
          {flavorText && <span key={shortid.generate()}>{flavorText}</span>}
